...
export default function FilterPartners(props: Props) {
  ...
  const [filteredCategory, setFilteredCategory] = useState<
    string | undefined
  >();
  const [partnersFiltered, setPartnersFiltered] = useState<PartnerProps[]>(
    partners,
  );
  ...
  const handlerFilterPerTypes = (type: TypesFilter, search: string) => {
    ...
    switch (type) {
      case "category":
        ...
        filtered = partnersFiltered.filter(({ category }) =>
          filteringTextValue(category, search)
        );
        break;

      case "country":
        setFilteredCountry(search);
        filtered = partnersFiltered.filter(({ country }) =>
          filteringTextValue(country, search)
        );
        break;
  ...
  const handlerFilterAll = (type: TypesFilter) => {
    ...
    if (type == "category" && filteredCountry) {
      ...
      filtered = partnersFiltered.filter(({ country }) =>
        filteringTextValue(country, filteredCountry)
      );
    } else if (type == "country" && filteredCategory) {
      setFilteredCountry(undefined);
      filtered = partnersFiltered.filter(({ category }) =>
        filteringTextValue(category, filteredCategory)
      );
    } else {
      filtered = partners;
    }
  ...
  const handlerFilterPerName = (search: string) => {
    let filtered: PartnerProps[] = [];
    filtered = partners.filter(({ category, country, name }) =>
  ...
  return (
    <div className="relative text-landing-primary">
      <div
        id="filterArea"
        ref={filterArea}
        className="pt-12 pb-12 w-screen absolute top-0 left-0"
        style={{
          backgroundColor: `${
      ...
      >
        <Container>
          ...
          <div className={showFilter ? "block" : "hidden"}>
            <button
              onClick={() =>
                handlerFilterAll("category")}
            >
              All
            </button>
            {partners.map((item) => (
              <button
                onClick={() => handlerFilterPerTypes("category", item.category)}
              >
                {item.category}
              </button>
            ))}
          ...
          <div className={showFilter ? "block" : "hidden"}>
            ...
            {partners.map((item) => (
              <button
                onClick={() => handlerFilterPerTypes("country", item.country)}
              >
                {item.category}
              </button>
            ))}
